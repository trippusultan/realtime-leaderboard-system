...
// ── Bootstrap CJS deps once at module load ───────────────────────────────────
const { default: _jwt } = await import("jsonwebtoken"); const jwtSign = _jwt.sign;
const { default: _bc } = await import("bcryptjs"); const bcryptCompare = _bc.compare;
...
router.post("/register", registerBody, async (req, res, next) => {
  try {
    ...
    const token = await jwtSign({ id }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
...
router.post("/login", loginBody, async (req, res, next) => {
  try {
    ...
    const valid = await bcryptCompare(password, userRecord.password_hash);
    if (!valid) return ok(res).unauthorized({ field: "password", message: "Invalid credentials" }).send();

    const token = await jwtSign({ id: userRecord.id }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
